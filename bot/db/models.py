...
class User(Base):
    ...
    blunders: Mapped[list["Blunder"]] = relationship(
        "Blunder", back_populates="user", cascade="all, delete-orphan"
    )
    opening_stats: Mapped[list["UserOpeningStat"]] = relationship(
        "UserOpeningStat", back_populates="user", cascade="all, delete-orphan"
    )
...
class UserOpeningStat(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    chesscom_username: Mapped[str] = mapped_column(
        String, ForeignKey("users.chesscom_username"), nullable=False
    )
